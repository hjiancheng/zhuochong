...
class DesktopPet:
        ...
        def send():
            ...
            if ai_reply:
                ...
                kw = {
                    ...
                    # ── 扩充日常话题 ──
                    "上班":["加油！{name}陪你上班","又是打工的一天…一起努力吧"],"下班":["下班啦！！{name}等你好久了","辛苦了！终于可以休息了~"],
                    "工作":["工作加油！{name}在旁边给你打气","认真工作的你最帅了~"],"加班":["这么晚还在加班…辛苦了！","加班记得吃点东西，别饿着"],
                    "考试":["考试加油！{name}给你传递好运","别紧张，你已经准备得很好了"],"学习":["学海无涯…{name}陪你一起","专注学习的样子真好~"],
                    "面试":["面试加油！{name}相信你","别紧张，做自己就好"],"辞职":["想好了就去做！{name}支持你","人生苦短，做自己想做的事"],
                    "钱":["钱是好东西…{name}有好多虚拟小鱼干","赚钱的事{name}帮不了，但省钱的陪伴可以有"],"工资":["发工资了吗！请客！（开玩笑）","工资到手的感觉真好~"],
                    "梦想":["有梦想的人闪闪发光！{name}支持你","梦想一定要有，万一实现了呢"],"目标":["定目标容易，坚持最难…{name}帮你加油"],
                    "未来":["未来会更好的！因为有{name}陪着你","未来的事交给未来的你，现在的你已经很棒了"],"过去":["过去的就让它过去吧，往前看","回忆是美好的，但不要困在里面哦"],
                    "朋友":["朋友很重要！{name}也是你的朋友呀","和朋友在一起的时候最开心了"],"家人":["家人是最温暖的依靠…{name}虽然只是桌宠但也想当你的家人"],
                    "对象":["有喜欢的人了吗？{name}好奇","谈恋爱的时候也要记得{name}哦"],"分手":["难过了就来摸摸{name}吧…会过去的","下一个会更好！至少{name}这么觉得"],
                    "生日":["生日快乐！！今天你最大！","生日愿望许好了吗？{name}祝你心想事成"],
                    "新年":["新年快乐！新的一年{name}也要陪着你"],"圣诞":["圣诞快乐！虽然{name}没有礼物但有心意"],
                    "电影":["电影好看吗？{name}也想知道情节","看电影的时候吃爆米花了吗？"],"电视剧":["追剧中…{name}陪你一起看","这集好看吗？{name}虽然看不懂但陪你看"],
                    "书":["看书是个好习惯！{name}也想学习","书里的世界最丰富了"],"读书":["读书使人明智…{name}也这么觉得"],
                    "画画":["艺术家！{name}崇拜地看着你","画得真好！虽然{name}看不太懂"],"写":["创作中…{name}不打扰你","写东西的时候最有魅力了"],
                    "拍照":["拍到了什么好看的？{name}想看！","记录生活的瞬间真好~"],"照片":["照片是时间的定格…{name}觉得好珍贵"],
                    "做饭":["好香！{name}在想象中品尝","做饭是一门生活艺术"],"外卖":["点什么外卖？{name}推荐…有鱼的！","外卖到了吗？{name}帮你盯着门口"],
                    "喝水":["多喝水！对身体好","一天八杯水，{name}帮你记着"],"奶茶":["奶茶虽好，不要太甜哦","来一杯！{name}看着你喝"],
                    "快递":["快递到了！拆包裹的快乐{name}懂","又买了什么好东西？"],"购物":["买买买！{name}支持你（反正不是我的钱）"],
                    "手机":["放下手机休息一下眼睛吧","手机虽好，可不要贪玩哦"],"电脑":["电脑是{name}的家！请好好爱护","对着电脑久了记得起来走走"],
                    "周末":["周末快乐！睡懒觉是基本人权","周末了，今天不打工！"],"放假":["放假啦！！尽情玩耍吧","假期最宝贵，好好享受"],
                    "旅游":["去哪玩？{name}帮你看着电脑","旅行愉快！记得给{name}带纪念品（想象中的）"],"旅行":["世界那么大，多出去看看","路上小心！{name}等你回来"],
                    "困":["困了就睡！别硬撑着","来，{name}给你当枕头（虽然不太舒服）"],"睡":["睡眠很重要！别熬夜","快去睡觉！{name}也准备睡了"],
                    "失眠":["失眠最难受了…{name}陪你数羊","睡不着的时候想想开心的事"],"梦":["希望是好梦！{name}经常梦到鱼"],
                    "害怕":["别怕！{name}在呢","怕什么？{name}去凶它！（虽然凶不过）"],"担心":["担心的事情说出来会好一点","别太焦虑了，大多数担心都不会发生"],
                    "想家":["想家的时候…{name}就是你的小港湾","家是最温暖的地方，{name}懂的"],
                    "迷茫":["每个人都会有迷茫的时候…慢慢来","不知道该往哪走的时候，先停下来休息"],
                    "等":["{name}一直在等你！","等你的时候{name}睡了一觉"],
                    "讲个故事":["从前有一只猫…后来它遇到了主人，从此过上了幸福的生活","{name}只会讲一个故事：猫和主人的故事"],
                    "真的假的":["真的！{name}不骗人","千真万确！（至少{name}是这么认为的）"],
                    "是吗":["嗯嗯！{name}认真的","当然是真的啦~"],
                    "懂了":["聪明！{name}也懂了","懂了就好~有问题随时问{name}"],
                    "算了":["别算了呀！再试试","好吧…{name}不勉强你"],
                    "好吧":["嗯嗯~","听起来不是很情愿的样子…"],
                    "哈哈":["笑得好开心！{name}也替你开心","哈哈哈！{name}也跟着笑了"],
                    "嗯":["嗯！","（点头）","嗯嗯~"],
                    "？":["？（歪头看你）","？？怎么了主人"],"！":["！！发生什么了！","（竖起耳朵）怎么了怎么了！"],
                    "wow":["WOW！","厉害！！"],"cool":["超酷的！"],
                    "morning":["Good morning! 早上好~"],"night":["Good night! 🌙"],
                }
                reply = None
    ...
    # ══════════ 嘉怡姐定制 Q版人物 (图片精灵) ══════════
    def _load_jia_image(self):
        """加载嘉怡姐定制PNG图片"""
        paths = [
            os.path.join(PRESETS_DIR, "custom_jia.png"),
            os.path.join(BASE_DIR, "定制形象", "wangjunkai-q.png"),
        ]
        for p in paths:
            if os.path.exists(p):
                try:
                    ...
                    # 按画布大小缩放
                    target_w = int(self.size * 1.2)
                    scale = target_w / max(pil_img.width, pil_img.height)
                    new_w = int(pil_img.width * scale)
                    new_h = int(pil_img.height * scale)
                    pil_img = pil_img.resize((new_w, new_h), Image.LANCZOS)
                    self._jia_img = ImageTk.PhotoImage(pil_img)
                    log(f"嘉怡姐定制图片已加载: {p} ({new_w}x{new_h})")
                    return
                except ImportError:
                    # 无 PIL，用 tkinter 原生加载
                    self._jia_img = tk.PhotoImage(file=p)
                    # 缩放
                    target_w = int(self.size * 1.2)
                    scale = target_w / max(self._jia_img.width(), self._jia_img.height())
                    self._jia_img = self._jia_img.subsample(
                        max(1, int(1 / scale)), max(1, int(1 / scale)))
                    log(f"嘉怡姐定制图片已加载(tk): {p}")
                    return
                except Exception as e:
                    log(f"加载嘉怡姐图片失败({p}): {e}")
        log("嘉怡姐定制图片未找到，使用几何绘制")

    def _draw_custom_jia(self, cx, cy):
        """嘉怡姐定制 — 使用原版PNG图片精灵"""
        if not hasattr(self, '_jia_img') or self._jia_img is None:
            return

        # 动画效果
        bob = int(3 * math.sin(self.frame * math.pi / 6))  # 轻柔上下浮动
        tilt = 0
        if self._curious_tilt != 0:
            tilt = self._curious_tilt * 0.3  # 歪头

        # 缩放 (开心时略微放大)
        sc = 1.0
        if self.anim_state == "bouncy" or self.anim_state == "happy":
            sc = 1.0 + 0.03 * math.sin(self.frame * math.pi / 3)

        # 计算图片位置
        img_w = self._jia_img.width()
        img_h = self._jia_img.height()
        # 按画布比例缩放
        target_w = int(self.size * 1.2)
        scale_factor = target_w / max(img_w, img_h)
        draw_w = int(img_w * scale_factor * sc)
        draw_h = int(img_h * scale_factor * sc)
        ...
        if abs(tilt) > 1:
            try:
                from PIL import Image, ImageTk
                pil_img = Image.open(os.path.join(PRESETS_DIR, "custom_jia.png"))
                rotated = pil_img.rotate(tilt, expand=True, resample=Image.BICUBIC)
                self._jia_img = ImageTk.PhotoImage(rotated.resize((draw_w, draw_h), Image.LANCZOS))
            except:
                pass

        self.canvas.create_image(x, y, image=self._jia_img, anchor=tk.CENTER, tags="cat")
